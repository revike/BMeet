...
class TestBoardConsumers(IsolatedAsyncioTestCase):
    ...
    async def test_board_consumer_anonymous_user(self):
        ...
        communicator = WebsocketCommunicator(websockets, "/api/board/1/")
    ...
    async def test_board_consumer_user(self):
        ...
        user, token = await self.get_user_token(user_id=2)
        communicator = WebsocketCommunicator(websockets, f"/api/board/2/?token={token.key}")
        connected, subprotocol = await communicator.connect()
        self.assertTrue(connected)
        self.assertEquals(subprotocol, None)
        self.assertEquals(communicator.scope['user'], user)
    ...
    @staticmethod
    @database_sync_to_async
    def get_initial_board_data(board_id, user_id):
        ...
        return board_data, redo_object, undo_object
